...
//     printf("Entering gFLDSplit::FillRates routine\n");
  ...
  float pHIconst = clight*TimeUnits*intSigESigHInu/hplanck/intSigE;
  for (i=0; i<size; i++)  phHI[i] = Er[i]*ErUn*pHIconst;

  // fill HeI and HeII photo-ionization rates
  float pHeIconst  = clight*TimeUnits*intSigESigHeInu/hplanck/intSigE;
  float pHeIIconst = clight*TimeUnits*intSigESigHeIInu/hplanck/intSigE;
  ...
  float GHIconst   = phScale*(intSigESigHI   - 13.6*ev2erg/hplanck*intSigESigHInu);
  float GHeIconst  = phScale*(intSigESigHeI  - 24.6*ev2erg/hplanck*intSigESigHeInu);
  float GHeIIconst = phScale*(intSigESigHeII - 54.4*ev2erg/hplanck*intSigESigHeIInu);
  if (Nchem == 1)

...
\item {\tt AMRFLDSourceGroupEnergy[:][:]} -- may be used in lieu of the
  two previous parameters, \\
  {\tt AMRFLDSourceType[:]} and {\tt AMRFLDSourceEnergy[:]}.
  Specifies a floating-point value for the total energy of a specific
  source for a specific frequency/group (in physical units of ergs
  cm$^{-3}$ s$^{-1}$).  The first index is the source index, the
  second is the frequency/group index.
\end{itemize}

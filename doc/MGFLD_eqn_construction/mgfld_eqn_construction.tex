...
\label{eq:multifrequency_photoionization}
   ...
   \sum_{\omega=1}^{N_f} \frac{c\, E_{\omega}\, \sigma_{\mn_i}(\nu_{\omega})}{h\nu_{\omega}^2}, \qquad i=1,\ldots,N_\text{chem},
\end{equation}
...
\notag
   &= 
   \frac{c}{\rhob} \sum_{i=1}^{N_\text{chem}} \sum_{\omega=1}^{N_f}\, \mn_i(t,\rvec)\,
     \sigma_{\mn_i}(\nu_{\omega}) E_{\omega}(t,\rvec) \left(\frac{1}{\nu_{\omega}} - \frac{\bar{\nu}_i}{\nu_{\omega}^2}\right).
\end{align}
...
\label{eq:square_omega_normalized_discretized}
  (\tilde{\square})_{\omega,i,j,k} = \begin{cases}
     ...
     \frac{\tAdot\, \nu_{\omega,R}}{2\tA}\left(\frac{\tE_{\omega,i,j,k}}{|g_{\omega}|} + \frac{\tE_{\omega+1,i,j,k}}{|g_{\omega+1}|}\right) -
      ...
      \text{if}\quad \nu_{\omega-1,R}\ne\nu_{\omega,L},\; \nu_{\omega,R}=\nu_{\omega+1,L},\\
     \frac{\tAdot\, \tE_{\omega,i,j,k}}{\tA\, |g_{\omega}|}\left(\nu_{\omega,R} - \nu_{\omega,L}\right),&
      \text{if}\quad \nu_{\omega-1,R}\ne\nu_{\omega,L},\; \nu_{\omega,R}\ne\nu_{\omega+1,L}.
  \end{cases}
\end{equation}
...
source: {\tt AMRFLDSourceType}+{\tt AMRFLDSourceEnergy} \underline{or}
{\tt AMRFLDSourceGroupEnergy}.  If the former mechanism is used, both
values must be specified for the source; if the latter is used it must
specify a non-negative energy for that source's contribution to 
{\em all} radiation fields.  One of these two approaches must be
...
\item {\tt AMRFLDSourceEnergy[:]} -- floating-point value specifying
  ...
  combination with {\tt AMRFLDSourceType[:]}.  At solver
...
\item {\tt AMRFLDSourceGroupEnergy[:][:]} -- floating-point value for
  the total energy of a specific source for a specific frequency/group
  (in physical units of ergs cm$^{-3}$ s$^{-1}$).  The first index is
  the source index, the second is the frequency/group index.

...
/
/  PURPOSE:
/    The PotentialField can have boundary points that will not be set
/      by the grid points on this level.  This routine sets those
/      points using a first-order interpolation from the parent grid.
/
/    Note: this is a nearly direct clone of 
...
int grid::CopyParentToPotentialFieldBoundary(grid *ParentGrid)
{
  ...
  // Interpolate (nearest neighbour).  To avoid active zones on this grid, 
  // we must do this on a face-by-face basis
  if(ParentGrid->PotentialField == NULL) ENZO_FAIL("NO PF in PARENT");
  int iparent, jparent, kparent, parentindex;
  // k-low
  for (k = 0; k < SubGridExtra[2]; k++) {
	...
	PotentialField[gravityindex] =
	  ParentGrid->PotentialField[parentindex+iparent];
	
      }
    }
  }
  // k-high
  for (k = GravitatingMassFieldDimension[2]-SubGridExtra[2]; 
       k < GravitatingMassFieldDimension[2]; k++) {
    kparent = nint((k + ParentOffset[2])/Refinement[2]);
    for (j = 0; j < GravitatingMassFieldDimension[1]; j++) {
      jparent = nint((j + ParentOffset[1])/Refinement[1]);
      parentindex = (kparent*ParentDim[1] + jparent)*ParentDim[0];
  ...
  for (k = 0; k < GravitatingMassFieldDimension[2]; k++) {
    kparent = nint((k + ParentOffset[2])/Refinement[2]);
    for (j = GravitatingMassFieldDimension[1]-SubGridExtra[1]; 
	 j < GravitatingMassFieldDimension[1]; j++) {
      jparent = nint((j + ParentOffset[1])/Refinement[1]);
      parentindex = (kparent*ParentDim[1] + jparent)*ParentDim[0];
      gravityindex = (k*GravitatingMassFieldDimension[1] + j)*
	GravitatingMassFieldDimension[0];
  ...
  for (k = 0; k < GravitatingMassFieldDimension[2]; k++) {
    ...
    for (j = 0; j < GravitatingMassFieldDimension[1]; j++) {
      jparent = nint((j + ParentOffset[1])/Refinement[1]);
      for (i = 0; i < SubGridExtra[0]; i++) {
	iparent = nint((i+ParentOffset[0])/Refinement[0]);
	PotentialField[(k*GravitatingMassFieldDimension[1] + j)*
		       GravitatingMassFieldDimension[0]+i] =
	  ParentGrid->PotentialField[(kparent*ParentDim[1]+jparent)*ParentDim[0]+iparent];
  ...
  for (k = 0; k < GravitatingMassFieldDimension[2]; k++) {
    ...
    for (j = 0; j < GravitatingMassFieldDimension[1]; j++) {
      jparent = nint((j + ParentOffset[1])/Refinement[1]);
      for (i = GravitatingMassFieldDimension[0]-SubGridExtra[0]; 
	   i < GravitatingMassFieldDimension[0]; i++) {
	iparent = nint((i+ParentOffset[0])/Refinement[0]);
	PotentialField[(k*GravitatingMassFieldDimension[1] + j)*
		       GravitatingMassFieldDimension[0]+i] =
	  ParentGrid->PotentialField[(kparent*ParentDim[1]+jparent)*ParentDim[0]+iparent];

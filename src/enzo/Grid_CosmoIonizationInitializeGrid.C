...
//     printf("Entering grid::CosmoIonizationInitializeGrid routine\n");
  ...
  int RhoNum, TENum, GENum, V0Num, V1Num, V2Num, DeNum, HINum, HIINum, 
  ...
  // if using the AMRFLDSplit solver, set the emissivity fields
  ...
  if (NewData == TRUE) {
    ...
    for (i=0; i<size; i++)  BaryonField[RhoNum][i]  = rhoConstant/DensityUnits;
    ...
    if (DualEnergyFormalism)
      for (i=0; i<size; i++)  BaryonField[GENum][i] = IEConstant/eUnits;
    // if using external chemistry/cooling, set rate fields
    if (RadiativeCooling) {

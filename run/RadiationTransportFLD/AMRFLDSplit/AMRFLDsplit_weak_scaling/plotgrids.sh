...
do
    ...
    then
	yt plot -p -f $FIELD -a $AXIS --show-grids DD000$i/data000$i
    else
	break
    fi
done
...
do
    ...
    then
	yt plot -p -f $FIELD -a $AXIS --show-grids DD00$i/data00$i
    else
	break
    fi
done
...
do
    if [ -f DD0$i/data0$i ]
    then
	yt plot -p -f $FIELD -a $AXIS --show-grids DD0$i/data0$i
    else
	break
    fi
done
...
do
    if [ -f DD$i/data$i ]
    then
	yt plot -p -f $FIELD -a $AXIS --show-grids DD$i/data$i
    else
	break
    fi
done

...
\usepackage{quoting}
\usepackage[threshold=5, thresholdtype=words]{csquotes}
...
%% New SED emission types, but with locations and energy handled by
%% AMR-FLD, which will perform integration to decompose among radiation
%% groups and fill emissivity arrays.

%% Additional radiation and emissivity fields, in case 10 radiation
%% fields is insufficient for a specific problem.


%% Finally, we note that the following existing {\tt ProblemTypes}
%% already support use of the AMR-FLD solver module:
%% \begin{itemize}
%% \item 
%% \item 
%% \item 
...
%% However, extension of other problem types to allow use of the AMR-FLD
%% solver module requires only a few steps.

%% Fields -- the problem initializer or provided restart file must
...
%% Fully user-supplied emissivities, in which the set of emission sources
%% is unknown at run-time and cannot be specified in the input parameter
%% file, but is determined on-the-fly based on the current physics.  The
%% most notable example of this is in using Enzo's existing star-maker
%% recipes to seed the radiaton solver.  At present, none of the
%% star-maker recipes have been updated to interface with the AMR-FLD
%% solver, so it is currently the user's responsibility for filling the
%% emissivity fields {\tt Emissivity0} $\to$ {\tt Emissivity9}.  These should be
